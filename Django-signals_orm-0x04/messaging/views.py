...
#     return render(request, 'conversations/messages.html', {'messages': message})
    ...
    if request.method == 'POST':
        ...
        if content:
            Message.objects.create(
                sender=request.user,
                receiver=conversation_owner,
                content=content
            )
            messages.success(request, "Message sent.")
            return redirect('conversation_view', conversation_owner=conversation_owner)
...
@login_required
@cache_page(60)
def conversation_messages(request, conversation_pk):
    messages = Message.objects.filter(conversation_id=conversation_pk).order_by('sent_at')
